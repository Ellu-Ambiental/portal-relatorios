...
function SupaAPI() {
  ...
  return {
    ...
    relatorios() {
      return todos(() => sb.from('relatorios').select('*, cliente:clientes(nome)')
        .order('data_emissao', { ascending: false }).order('numero').order('revisao', { ascending: false }));
    },
    async abrirUrl(r) { return ok(await sb.storage.from('relatorios').createSignedUrl(r.arquivo_path, 300)).signedUrl; },
    async blob(r) { return ok(await sb.storage.from('relatorios').download(r.arquivo_path)); },
    async log(r, acao) { try { await sb.from('acessos_log').insert({ relatorio_id: r.id, user_id: await uid(), acao }); } catch (_) {} },
    ...
    async convidar(dados) {
      const { data, error } = await sb.functions.invoke('criar-usuario', { body: { ...dados, redirectTo: baseUrl() } });
      if (error) { let msg = error.message; try { msg = (await error.context.json()).error || msg; } catch (_) {} throw new Error(msg); }
      if (data?.error) throw new Error(data.error);
      return data;
    },
    async publicar(meta, arquivo, hash) {
      const path = `${meta.cliente_id}/${meta.numero.replace(/[^\w.-]+/g, '_')}_rev${meta.revisao}_${Date.now()}.pdf`;
      ok(await sb.storage.from('relatorios').upload(path, arquivo, { contentType: 'application/pdf', upsert: false }));
      const { data, error } = await sb.from('relatorios').insert({
        ...meta, arquivo_path: path, arquivo_nome: arquivo.name, tamanho_bytes: arquivo.size,
        hash_sha256: hash, publicado_por: await uid(),
      }).select('*, cliente:clientes(nome)').single();
      if (error) { await sb.storage.from('relatorios').remove([path]); throw error; }
      return data;
    },
...
function DemoAPI() {
  ...
  for (let i = 0; i < 22; i++) {
    ...
    rel.push({
      ...
      arquivo_path: 'r' + i, tamanho_bytes: 180000 + i * 7311, codigo_validacao: cod(), created_at: dt.toISOString(),
    });
  }
  // uma revisão de exemplo
  const orig = rel[2]; orig.status = 'substituido';
  rel.unshift({ ...orig, id: 'r2b', revisao: 1, status: 'vigente', arquivo_path: 'r2b', codigo_validacao: cod(), motivo_revisao: 'Correção do ponto de coleta', data_emissao: orig.data_emissao });
  rel[8].status = 'cancelado';
  const pronto = Promise.all(rel.map(async r => {
    const b = pdf(`${CFG.EMPRESA || 'Ellu Ambiental'} - RELATORIO DE ENSAIO\n\nNumero: ${r.numero}  Revisao: ${r.revisao}\nCliente: ${r.cliente.nome}\nPonto: ${r.ponto_coleta}\nMatriz: ${r.matriz}\nColeta: ${fmtData(r.data_coleta)}\n\nCodigo de validacao: ${r.codigo_validacao}\n\n(Documento ficticio - modo demonstracao)`);
    arquivos.set(r.arquivo_path, b); r.hash_sha256 = await sha256(b);
  }));
  const log = [];
  let atual = null;
  ...
  const pub = r => {
    ...
    return { numero: r.numero, revisao: r.revisao, cliente: c.nome, data_emissao: r.data_emissao, data_coleta: r.data_coleta, status: r.status, hash_sha256: r.hash_sha256,
  ...
  return {
    ...
    async relatorios() {
      ...
      return clone(vis).sort((a, b) => b.data_emissao.localeCompare(a.data_emissao) || a.numero.localeCompare(b.numero) || b.revisao - a.revisao);
    },
    async abrirUrl(r) { return URL.createObjectURL(arquivos.get(r.arquivo_path)); },
    async blob(r) { return arquivos.get(r.arquivo_path); },
    async log(r, acao) { log.unshift({ id: log.length + 1, relatorio_id: r.id, user_id: atual.id, acao, created_at: new Date().toISOString() }); },
    async validar(c) { await pronto; return pub(rel.find(r => r.codigo_validacao === c.trim().toUpperCase())) || null; },
    async validarHash(h) { await pronto; return pub(rel.find(r => r.hash_sha256 === h)) || null; },
    ...
    async convidar(d) {
      ...
      perfis.push({ id: 'u' + (perfis.length + 1), ...d, cliente_id: d.papel === 'admin' ? null : d.cliente_id, ativo: true, created_at: new Date().toISOString() });
    },
    async publicar(meta, arquivo, hash) {
      if (rel.some(r => r.numero === meta.numero && r.revisao === meta.revisao)) throw new Error('duplicate key numero');
      const r = { id: 'r' + Date.now(), ...meta, arquivo_path: 'n' + Date.now(), arquivo_nome: arquivo.name, tamanho_bytes: arquivo.size,
        hash_sha256: hash, status: 'vigente', codigo_validacao: meta.codigo_validacao || cod(), created_at: new Date().toISOString(),
        cliente: { nome: clientes.find(c => c.id === meta.cliente_id).nome } };
      arquivos.set(r.arquivo_path, arquivo);
      rel.forEach(x => { if (x.numero === r.numero && x.revisao < r.revisao && x.status === 'vigente') x.status = 'substituido'; });
      rel.unshift(r); return clone(r);
    },
...
const S = {
  ...
  sort: { k: 'data_emissao', dir: -1 }, pagina: 1, sel: new Set(), aba: 'relatorios',
};
...
function viewValidar(codigoInicial = '') {
  ...
  app().innerHTML = `${demoFlag()}<div class="auth-wrap"><div class="auth-card largo">
    ...
    <div class="drop" id="drop-val">${ic('file')}<div><b>Arraste o PDF aqui</b> ou clique para selecionar</div>
      <small>O arquivo não é enviado — apenas sua impressão digital (SHA-256) é comparada.</small>
      <input type="file" accept="application/pdf" hidden></div>
    <div id="res-val"></div>
  ...
  const mostrar = (r, origem) => {
    if (!r) {
      res.innerHTML = `<div class="valid-res erro"><h3>${ic('xc')} ${origem === 'hash' ? 'Arquivo não reconhecido' : 'Código não encontrado'}</h3>
        <p style="margin:0">${origem === 'hash' ? 'Este PDF não corresponde a nenhum relatório emitido — ele pode ter sido alterado.' : 'Confira o código digitado. Se o problema persistir, entre em contato com a Éllu Ambiental.'}</p></div>`;
      return;
    }
    ...
    res.innerHTML = `<div class="valid-res ${cls}"><h3>${tit}</h3><dl>
      ...
      <dt>SHA-256</dt><dd><span class="codigo" style="font-size:11px">${esc(r.hash_sha256)}</span></dd></dl>
      ${origem === 'hash' ? '<p style="margin:10px 0 0;font-size:13px">O arquivo é idêntico ao emitido pelo laboratório.</p>' : ''}</div>`;
  };
...
function ligarDrop(el, onFile) {
  ...
  el.ondrop = e => {
    ...
    if (f && (f.type === 'application/pdf' || /\.pdf$/i.test(f.name))) onFile(f); else toast('Selecione um arquivo PDF.', 'erro');
  };
}
  ...
  $('#lista').innerHTML = !l.length
    ...
    : `<div class="tb-wrap"><table class="tb resp"><thead><tr>
      ...
      ${pag.map(r => `<tr data-id="${r.id}">
        ...
        <td data-l="Relatório"><div class="num">${esc(r.numero)}</div><div class="sub">Rev. ${r.revisao}${r.titulo ? ' · ' + esc(r.titulo) : ''}</div></td>
        ...
        <td class="acoes">
          <button class="icon-btn" data-a="ver" title="Visualizar">${ic('eye')}</button>
          <button class="icon-btn" data-a="baixar" title="Baixar PDF">${ic('download')}</button>
...
async function acao(a, r, btn) {
  try {
    if (a === 'ver') {
      ...
      const url = await API.abrirUrl(r);
      if (w) w.location = url; else location.href = url;
      API.log(r, 'visualizar');
    } else if (a === 'baixar') {
      await comBotao(btn, async () => { baixarBlob(await API.blob(r), nomeArquivo(r)); API.log(r, 'download'); });
...
async function baixarZip(btn) {
  const itens = S.relatorios.filter(r => S.sel.has(r.id));
  await comBotao(btn, async () => {
    try {
      const zip = new JSZip();
      for (const r of itens) { zip.file(nomeArquivo(r), await API.blob(r)); API.log(r, 'download'); }
      baixarBlob(await zip.generateAsync({ type: 'blob' }), `relatorios_${hoje()}.zip`);
      toast(`${itens.length} relatório(s) baixado(s).`, 'ok');
    } catch (e) { toast(traduzErro(e), 'erro'); }
  });
}
...
function modalInfo(r, titulo = 'Detalhes do relatório') {
  ...
  const m = modal(titulo, `
    ...
    <div class="valid-res" style="background:#fafbfc;border:1px solid var(--borda)"><dl>
      ...
      ${r.motivo_revisao ? `<dt>Motivo da revisão</dt><dd>${esc(r.motivo_revisao)}</dd>` : ''}
      <dt>Tamanho</dt><dd>${fmtBytes(r.tamanho_bytes)}</dd>
      <dt>SHA-256</dt><dd><span class="codigo" style="font-size:11px">${esc(r.hash_sha256 || '')}</span></dd></dl></div>`,
    `<button class="btn" data-fechar>Fechar</button><button class="btn btn-pri" id="dl">${ic('download')} Baixar PDF</button>`);
...
function renderAba() {
  ...
  if (S.aba === 'relatorios') {
    ...
    aba.innerHTML = `<div class="card"><div class="tb-wrap"><table class="tb resp"><thead><tr><th>Nome</th><th>E-mail</th><th>Cliente</th><th>Perfil</th><th>Situação</th><th></th></tr></thead><tbody>
      ${S.perfis.map(p => `<tr data-id="${p.id}"><td data-l="Nome"><b>${esc(p.nome || '—')}</b></td><td data-l="E-mail">${esc(p.email || '')}</td>
        ...
        <td class="acoes"><button class="icon-btn" data-a="edit" title="Editar">${ic('edit')}</button></td></tr>`).join('')}</tbody></table></div></div>`;
    $$('#aba tr[data-id]').forEach(tr => { $('[data-a=edit]', tr).onclick = () => modalUsuario(S.perfis.find(p => p.id === tr.dataset.id)); });
    ...
    API.acessos().then(log => {
      ...
      aba.innerHTML = `<div class="card"><div class="tb-wrap"><table class="tb resp"><thead><tr><th>Data/hora</th><th>Usuário</th><th>Cliente</th><th>Relatório</th><th>Ação</th></tr></thead><tbody>
        ${log.map(l => { const u = usu(l.user_id), r = rel(l.relatorio_id); return `<tr><td data-l="Data/hora">${fmtDataHora(l.created_at)}</td>
          ...
          <td data-l="Relatório">${r ? esc(r.numero) + ' rev. ' + r.revisao : '—'}</td><td data-l="Ação">${l.acao === 'download' ? 'Download' : 'Visualização'}</td></tr>`; }).join('')
...
function modalPublicar(base = null) {
  ...
  const m = modal(base ? `Nova revisão — ${esc(base.numero)}` : 'Publicar relatório de ensaio', `
    <form id="f-pub" style="display:flex;flex-direction:column;gap:12px">
      ...
      <div class="drop" id="drop-pub">${ic('upload')}<div><b>Arraste o PDF do relatório</b> ou clique para selecionar</div><small>Máx. 50 MB</small><input type="file" accept="application/pdf" hidden></div>
  ...
  const drop = $('#drop-pub', m.el);
  const aoEscolher = async file => {
    if (file.size > 52428800) return toast('Arquivo acima de 50 MB.', 'erro');
    arquivo = file; hash = await sha256(file);
    const dup = S.relatorios.find(r => r.hash_sha256 === hash);
    drop.classList.add('ok');
    drop.innerHTML = `${ic('file')}<div><b>${esc(file.name)}</b> · ${fmtBytes(file.size)}</div><small>Clique para trocar</small><input type="file" accept="application/pdf" hidden>`;
    ligarDrop(drop, aoEscolher);
    if (dup) toast(`Atenção: este mesmo arquivo já foi publicado como ${dup.numero} rev. ${dup.revisao}.`, 'erro');
  };
  ligarDrop(drop, aoEscolher);
  ...
  $('#pub', m.el).onclick = async e => {
    ...
    if (!arquivo) return toast('Selecione o PDF do relatório.', 'erro');
    ...
    await comBotao(e.currentTarget, async () => {
      try {
        const novo = await API.publicar(meta, arquivo, hash);
